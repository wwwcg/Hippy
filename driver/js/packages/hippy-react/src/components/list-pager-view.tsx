...
interface ListViewProps {
  ...
  /**
   * Scroll to offset after `ListPagerView` with data rendered.
   */
  ...
  /**
   *  Called when the `ListPagerView` is scrolling to bottom.
   */
  onEndReached?: () => void;
  ...
  /**
   * Called when the momentum scroll starts (scroll which occurs as the ListPagerView starts gliding).
   */
  onMomentumScrollBegin?: () => void;

  /**
   * Called when the momentum scroll ends (scroll which occurs as the ListPagerView glides to a stop).
   */
  ...
  /**
   * Called when user pulls the ListPagerView down
   */
  onHeaderPulling?: () => void

  /**
   * Called when user release the pulling ListPagerView
   */
  onHeaderReleased?: () => void

  /**
   * Called when user swipe up ListPagerView to get more data on reaching the footer
   */
  onFooterPulling?: () => void

  /**
   * Called when user release the getting-more-data ListPagerView
   */
 ...
 * @noInheritDoc
 */
class ListPagerView extends React.Component<ListViewProps, ListViewState> {
  ...
  public componentDidMount() {
    ...
    if (!getRowKey) {
      warn('ListPagerView needs getRowKey to specific the key of item');
    }
  }
      ...
      for (let index = 0; index < numberOfRows; index += 1) {
        ...
        [
          ...
          if (typeof func === 'function') {
            itemProps[ListPagerView.convertName(name)] = () => {
              func(index);
            };
          }
        });
