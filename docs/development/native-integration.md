...
- 安装 [CocoaPods](https://cocoapods.org/)
  ...
  > 当然，也可以不使用CocoaPods来集成 Hippy iOS Framework，但这样会使集成过程变得复杂，您需要手动打包、添加库以及设置必要的链接参数到现有项目中。
...
1. 首先，确定要集成的Hippy iOS SDK版本（[版本查询地址](https://github.com/Tencent/Hippy/releases)），并记录以备后续使用
