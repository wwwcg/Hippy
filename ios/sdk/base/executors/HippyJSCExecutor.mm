...
static bool defaultDynamicLoadAction(const unicode_string_view& uri, std::function<void(u8string)> cb, HippyBridge *bridge) {
    ...
    if ([url isFileURL]) {
        ...
        __weak HippyJSCExecutor *weakExecutor = [bridge javaScriptExecutor];
        [[[NSURLSession sharedSession] dataTaskWithRequest:req completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
            HippyJSCExecutor *strongExecutor = weakExecutor;
            if (!strongExecutor) {
                return;
            }
            if (error) {
                HippyLogInfo(@"[Hippy_OC_Log][Dynamic_Load], error:%@", [error description]);
                [strongExecutor executeBlockOnJavaScriptQueue:^{
...
- (NSString *)uniqueExecutorkeyForEngine {
    /// engine reuse can lead to leak of context, which we avoid by using a unique executor key
